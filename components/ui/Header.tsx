...
export default function DashboardHeader() {
  ...
  const invitationNotifs = notifications.filter(
    (n) =>
      n.type === "invitation" && (n.status === "unread" || n.status === "read"),
  );
  const basicNotifs = notifications.filter(
    (n) =>
      n.type === "basic" || n.status === "accepted" || n.status === "rejected",
  );
      ...
      <div className="flex items-center justify-between gap-4 h-20 px-4 lg:px-8">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="relative">
            ...
            >
              <Bell
                className="h-5 w-5 text-slate-600 hover:text-emerald-600 transition-colors"
                strokeWidth={2.5}
              />
              {unreadCount > 0 && (
            ...
            {showNotifications && (
              <>
                <div
                  className="fixed left-4 right-4 top-20 z-50 sm:absolute sm:right-0 sm:left-auto sm:top-full sm:mt-2 sm:w-[420px] sm:rounded-2xl flex flex-col bg-white rounded-2xl border-2 border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.15)] max-h-[80vh] sm:max-h-[600px] overflow-hidden sm:shadow-xl animate-in fade-in-0 zoom-in-95 sm:fade-in-0 sm:zoom-in-100 duration-200"
                  style={{
                    fontFamily:
