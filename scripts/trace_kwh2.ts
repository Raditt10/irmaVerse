...
async function main() {
  ...
  if (!material) {
    ...
    console.log("Invites for KWH 2:", invites.map(i => ({ 
      id: i.id, 
    ...
    console.log("Notifications for KWH 2:", notifications.map(n => ({ 
      id: n.id, 
      user: n.user.email, 
      status: n.status, 
      type: n.type,
  ...
  const user = await prisma.user.findUnique({
    ...
    include: {
      ...
      materialinvite: { include: { material: true } }
    }
  })
  ...
  if (user) {
    ...
    console.log("Invites:", user.materialinvite.map(i => ({ 
