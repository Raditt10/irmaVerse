...
const EditMaterial = () => {
  ...
  useEffect(() => {
    if (formData.parentId) {
      fetchProgramSessions(formData.parentId);
    } else {
      setProgramSessions([]);
      ...
      try {
        ...
        if (material) {
          ...
          setFormData({
            ...
            parentId: material.programId || material.parentId || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { 
        ...
        programId: formData.parentId || null,
      };
                ...
                <div className="bg-white p-5 lg:p-8 rounded-3xl lg:rounded-[2.5rem] border-2 border-slate-200 shadow-[0_4px_0_0_#cbd5e1] lg:shadow-[0_8px_0_0_#cbd5e1]">
                  ...
                  <div className="space-y-4 lg:space-y-6">
                    ...
                    <div className="space-y-3">
                          ...
                          <div className="relative">
                            ...
                            >
                              <span className="truncate">
                                {availablePrograms.find(p => p.id === formData.parentId)?.title || "--- Pilih Program ---"}
                              </span>
                                  ...
                                  {fetchingPrograms ? (
                                    ...
                                    <>
                                      <button
                                        ...
                                        `}
                                      >
                                        Bukan bagian dari program (Mandiri)
                                      </button>
                                      {availablePrograms.map((program) => (
                                        <button
                                          ...
                                          type="button"
                                          onClick={() => {
                                            setFormData({ ...formData, parentId: program.id });
                                            setIsProgramDropdownOpen(false);
                                          }}
                                          className={`
                                            w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all
                                            ${formData.parentId === program.id 
                                              ? "bg-teal-50 text-teal-600" 
                      ...
                      {formData.category === "Program Wajib" && formData.parentId && (
                        <div className="mt-4 animate-in fade-in slide-in-from-top-2 duration-300">
                      ...
                      {formData.materialType === "editor" ? (
                        <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                          <Textarea
                            ...
                            onChange={(e) => setFormData({ ...formData, materialContent: e.target.value })}
