...
const MaterialDetail = () => {
  ...
  const getCategoryBadge = (category: string) => {
    ...
    if (category === "Program Wajib") style = "bg-rose-100 text-rose-700 border-rose-200";
    if (category === "Program Ekstra") style = "bg-purple-100 text-purple-700 border-purple-200";
    if (category === "Program Next Level") style = "bg-amber-100 text-amber-700 border-amber-200";

    return (
      <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black uppercase border-2 shadow-sm ${style}`}>
      ...
      <div className="min-h-screen bg-[#FDFBF7]">
        ...
        <div className="flex">
          ...
          <div className="flex-1 p-8 flex flex-col items-center justify-center min-h-[calc(100vh-80px)]">
            ...
            <h2 className="text-2xl font-black text-slate-700 mb-2">Kajian Tidak Ditemukan</h2>
            <p className="text-slate-500 mb-6">Mungkin kajian ini sudah dihapus atau ID-nya salah.</p>
            <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-[0_4px_0_0_#cbd5e1] flex flex-col items-center text-center hover:-translate-y-1 transition-transform">
                        <div className="w-12 h-12 rounded-2xl bg-teal-50 flex items-center justify-center mb-3 border-2 border-teal-100">
                            <Calendar className="h-6 w-6 text-teal-500" strokeWidth={2.5} />
                        </div>
                        ...
                        <span className="text-slate-800 font-black text-sm px-2 leading-tight">
                            {new Date(material.date).toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' })}
                        </span>
                    </div>
                    <div className="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-[0_4px_0_0_#cbd5e1] flex flex-col items-center text-center hover:-translate-y-1 transition-transform">
                        <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center mb-3 border-2 border-indigo-100">
                            <Clock className="h-6 w-6 text-indigo-500" strokeWidth={2.5} />
                        </div>
                        <span className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Waktu</span>
                        <span className="text-slate-800 font-black text-sm px-2 leading-tight">{material.startedAt} WIB</span>
                    </div>
                    <div className="bg-white p-5 rounded-3xl border-2 border-slate-200 shadow-[0_4px_0_0_#cbd5e1] flex flex-col items-center text-center hover:-translate-y-1 transition-transform">
                        ...
                        <span className="text-slate-800 font-black text-sm px-2 leading-tight">{material.location}</span>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 lg:p-8 rounded-[2.5rem] border-2 border-slate-200 shadow-[0_6px_0_0_#cbd5e1]">
                  <div className="flex items-center gap-4 mb-6">
                      <div className="p-2.5 bg-blue-100 rounded-2xl border-2 border-blue-200">
                          <BookOpen className="h-6 w-6 text-blue-600" strokeWidth={3} />
                      </div>
                      <h2 className="text-xl lg:text-2xl font-black text-slate-800">Detail Materi</h2>
                  </div>
              ...
              <div className="space-y-6 lg:space-y-8">
                ...
                <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-[0_6px_0_0_#cbd5e1] overflow-hidden p-6 lg:p-8 text-center">
                    <div className="w-28 h-28 mx-auto bg-slate-100 rounded-full mb-4 border-4 border-teal-100 overflow-hidden relative shadow-sm">
                         {material.instructorAvatar ? (
                             <img src={material.instructorAvatar} alt={material.instructor} className="w-full h-full object-cover" />
                         ) : (
                    ...
                    <h3 className="text-xl font-black text-slate-800 leading-tight mb-1">{material.instructor}</h3>
                    ...
                    <div className="space-y-3">
                        <button 
                           onClick={() => router.push(`/instructors/chat?name=${encodeURIComponent(material.instructor)}`)}
                ...
                {isPrivileged ? (
                    <div className="bg-slate-50 rounded-[2.5rem] p-6 text-center border-2 border-slate-200 border-dashed">
                        <p className="text-slate-500 text-sm font-bold mb-4">Kelola Kajian Ini</p>
                    ...
                    <div className="bg-linear-to-br from-teal-400 to-cyan-400 rounded-[2.5rem] p-6 lg:p-8 text-white border-2 border-teal-600 shadow-[0_6px_0_0_#0f766e] text-center relative overflow-hidden">
                        ...
                        <h3 className="text-2xl font-black mb-2 relative z-10">Siap Hadir?</h3>
                        <p className="text-teal-50 text-sm font-bold mb-6 leading-relaxed relative z-10">
                            Jangan lupa isi absensi saat kegiatan berlangsung untuk mencatat kehadiranmu.
                        </p>
                        <button 
